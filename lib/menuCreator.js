...
// його поля приходять з файлу запитів queries.
...
export function menuItemsMerger(
  ...
  const transformedSHSpecialities = itemsOrderAscTransform(
    specialitiesSHItems,
    transformedSpecialities[3].children
  );
  ...
  const transformedScienceFaAPofMTConference = itemsOrderAscTransform(
    scienceConferenceByIPulujItems
    // transformedScience[2].children
  );
  ...
  return {
    ...
    transformedScienceFaAPofMTConference,
...
export function menuCreator(
  ...
  const transformedSciencewSubs = transformedScience.map((item, index) => {
    ...
    if (index === 3) {
      return {
        ...item,
        children: [...transformedScienceFaAPofMTConference],
      };
    }
    return item;
  });
