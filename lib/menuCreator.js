...
export function menuItemsMerger(
  ...
  const transformedSHSpecialities = itemsOrderAscTransform(specialitiesSHItems, transformedSpecialities[3].children);

  const transformedBachelor = itemsOrderAscTransform(bachelorItems, menuItems[2].children);
  const transformedBachelorEPP = itemsOrderAscTransform(bachelorEPPItems, transformedBachelor[0].children);

  const transformedMaster = itemsOrderAscTransform(masterItems, menuItems[3].children);
  const transformedMasterEPP = itemsOrderAscTransform(masterEPPItems, transformedMaster[0].children);

  const transformedEntrant = itemsOrderAscTransform(entrantItems, menuItems[4].children);

  const transformedScience = itemsOrderAscTransform(scienceItems, menuItems[5].children);

  const transformedPostgraduate = itemsOrderAscTransform(postgraduateItems, menuItems[6].children);

  const transformedInternationalActivity = itemsOrderAscTransform(internationalActivityItems, menuItems[7].children);

  const transformedEducationalActivity = itemsOrderAscTransform(educationalActivityItems, menuItems[8].children);

  const transformedReferences = itemsOrderAscTransform(referencesItems, menuItems[9].children);

  const transformedIntresting = itemsOrderAscTransform(intrestingItems, menuItems[10].children);
...
export function menuCreator(
  ...
  // -----------------------------------------------------------------------------------------
  // Додаю підпункти до розділу меню 'Про кафедру'
  ...
  const transformedSpecialitieswSubs = transformedSpecialities.map((item, index) => {
    if (index === 3) {
      return {
        ...item,
