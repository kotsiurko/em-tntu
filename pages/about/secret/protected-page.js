...
const ProtectedPage = ({ secretDocsList }) => {
  ...
  const [selectedDocList, setSelectedDocList] = useState(
    JSON.parse(JSON.stringify(secretDocsList))
  );
  ...
  const handleInputSubmit = (event) => {
    ...
    if (searchQuery.length > 0) {
      ...
      const searchedSelectedDocList = tempArr
        .map((obj) => ({ ...obj, docs: obj.docs.slice() }))
        .filter((obj) => {
          obj.docs = obj.docs.filter(
            (doc) =>
              doc.docForWhom
                .toLowerCase()
  ...
  return (
    <>
      <Head>
        <title>
          Положення, розпорядження та накази | Кафедра електричної інженерії
          ТНТУ
        </title>
      </Head>
              ...
              return (
                <div className="card" key={_id} style={{ fontSize: "0.9rem" }}>
                  ...
                  <div
                    id="collapseOne"
                    className={
                      selectedItemId === _id ? "collapse show" : "collapse"
                    }
                          ...
                          {docs &&
                            docs.map((row) => {
                              return (
                                <tr key={row._key}>
                                  <td scope="row">
                                    {moment(row.publishedDate).format("DD.MM")}
                                  </td>
                                  <th scope="row">{row.docNumber}</th>
                                  <td scope="row">
                                    <a href={row.docUrl} download>
                                      {row.docTitle}
                                    </a>
                                  </td>
                                  <td scope="row">
...
export async function getStaticProps() {
  const secretDocsList = await client.fetch(
    `*[_type == "secretPage" && defined(docs)] | order(year desc)`
  );
